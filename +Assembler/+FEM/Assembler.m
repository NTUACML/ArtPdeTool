classdef Assembler < Assembler.AssemblerBase
    ...
    methods
        ...
        function status = Assembly(this, type, var, basis_id, data)
            ...
            if(type == AssemblyType.Matrix)
                status = this.FEM_LHS_Assembly(var, basis_id, data);
            elseif(type == AssemblyType.Vector)
                status = this.FEM_RHS_Assembly(var, basis_id, data);
            elseif(type == AssemblyType.Constraint)
                status = this.FEM_Constraint_Assembly(var, basis_id, data);
            else
                disp('Error <FEM Assembler>! - Assembly!');
    ...
    methods(Access = private)
        function status = FEM_LHS_Assembly(this, var, basis_id, data)
            if(length(var) == 2 && length(basis_id) == 2)
                ...
                disp('Error <FEM Assembler>! - FEM_LHS_Assembly!');
                disp('> LHS assemly error, please check!');
                status = false;
            end
        end
        
        function status = FEM_RHS_Assembly(this, var, basis_id, data)
            status = false;
        end
        
        function status = FEM_Constraint_Assembly(this, var, basis_id, data)
            if(size(data, 2) == 2)
                dof_id = data(:,1);
                const_value = data(:,2);
                ...
                disp('Error <FEM Assembler>! - FEM_Constraint_Assembly!');
